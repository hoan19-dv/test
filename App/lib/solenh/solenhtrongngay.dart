...
final class _solenhState extends State<solenh> {
  dynamic Custodycd =
      FlutterSecureStorage().read(key: 'Custodycd').then((value) => value);
  String? Acctno = '12345678';
  String? status;
  String? allowCancel;
  ...
  Future<void> filterData() async {
    try {
      ...
      if (selectedItems.join(',').isNotEmpty) {
        data.add('status=${selectedItems.join(',')}');
      }
      if (allowCancel != null && allowCancel!.isNotEmpty) {
        data.add('allowCancel=$allowCancel');
      }
      if (symbol.text.isNotEmpty) {
        data.add('symbol=${symbol.text}');
      }
      data.add('from=${1}');
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
        future: GetAllCode('API', 'ODORSTATUS',
                          ...
                          child: TextField(
                            ...
                            decoration: InputDecoration(
                              ...
                              prefixIcon: GestureDetector(
                                  child: Icon(Icons.search), onTap: filterData),
                              hintText: 'Mã CK',
                              hintStyle: TextStyle(
                                  fontSize: 10,
